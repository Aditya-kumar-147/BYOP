...
def main():
    ...
    html_temp = """
    <div style="background-color:teal ; padding:10px; border-radius: 5px;border-style: solid;margin-bottom: 30px;">
    <h2 style="color:#FFB741; text-align:center;">CVD Predictor Plus</h2>
    <h4 style="color:#D7D7D7; text-align:center;">"Heart Health in Your Hands: Predict, Prevent, Prevail!"</h4>
    </div>
    """
    ...
    if gender is 'Male':
        ...
        gender = 0
    height=st.number_input("Enter your height (cms)",value=164.35)
    weight=st.number_input("Enter your weight (Kgs)", value = 74.20)
    height = height/100
    ...
    ap_hi=st.number_input("Enter your systolic blood pressure (ap_hi)", value=128.81)
    ap_lo=st.number_input("Enter your diastolic blood pressure (ap_lo)", value=96.63)

    map = ( ( 2 * ap_lo) + ap_hi ) / 3
 
    years = st.number_input("Enter your age (Yrs)", step = 1)
    ...
    if chol is 'Normal':
        ...
        chol = 3
    gluc=st.radio("Select your Glucose Level", ['Normal','Above Normal','Well above Normal'])
    if gluc is 'Normal':
        gluc = 1
    ...
    if st.button('Predict'):#button name is Predict
